...
class GoogleCloudBackupService(QThread):
    ...
    finished_signal = Signal(dict)  # backup results
...
class BackupScheduler(QThread):
    ...
    def should_run_backup(self) -> bool:
        """Check if backup should run based on schedule with robust file handling"""
        ...
        # Get last backup time with fallback
        last_backup = None
        try:
            if os.path.exists(last_backup_file):
                ...
                home_backup_file = os.path.join(os.path.expanduser('~'), 'last_backup_time.txt')
                if os.path.exists(home_backup_file):
                    with open(home_backup_file, 'r') as f:
                        last_backup = datetime.fromisoformat(f.read().strip())
        except Exception as e:
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"Error reading last backup time: {e}")
            last_backup = None
    ...
    def update_last_backup_time(self):
        """Update the last backup timestamp with proper error handling"""
        try:
            with open('last_backup_time.txt', 'w') as f:
                f.write(datetime.now().isoformat())
        except Exception as e:
            # If we can't write to the file, log the error but don't crash
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"Could not update last backup time file: {e}")
            # Try to create the file in a different location
            try:
                import os
                backup_file = os.path.join(os.path.expanduser('~'), 'last_backup_time.txt')
                with open(backup_file, 'w') as f:
                    f.write(datetime.now().isoformat())
                logger.info(f"Created backup time file in home directory: {backup_file}")
            except Exception as e2:
                logger.error(f"Failed to create backup time file in home directory: {e2}")
