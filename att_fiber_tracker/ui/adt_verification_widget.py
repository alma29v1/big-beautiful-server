...
class ADTVerificationWidget(QWidget):
    ...
    def setup_controls(self, layout):
        ...
        self.load_nondetected_button = QPushButton("Load Non-Detected Images")
        self.load_nondetected_button.clicked.connect(self.load_nondetected_images)
        self.load_nondetected_button.setStyleSheet("background-color: #2196F3; color: white; font-weight: bold;")
        ...
        action_layout.addWidget(self.load_nondetected_button)
    ...
    def load_nondetected_images(self):
        """Load all analyzed images from adt_detections folder for verification"""
        try:
            import glob
            
            # Look for all property image directories
            image_dirs = glob.glob("adt_detections/*/")
            
            if not image_dirs:
                QMessageBox.warning(self, "No Images", "No ADT detection image directories found.\n\nRun ADT detection first to download property images.")
                return
            ...
            for image_dir in image_dirs:
                ...
                if len(parts) >= 2:
                    ...
                    city = "Unknown"
                    address = dir_name.replace('_', ' ')
                
                # Find images in this directory
                image_files = glob.glob(os.path.join(image_dir, "redfin_image_*.jpg"))
                
                if image_files:
                    # Use the first image for verification
                    ...
                    verification_record = {
                        ...
                        'confidence': 0.0,  # Default to no detection
                        'adt_detected': False,
                        'detection_method': 'Manual Verification',
                        'explanation': 'Image loaded for manual verification - AI found no ADT signs',
                        'timestamp': datetime.now().isoformat(),
                        'verification_status': 'pending',
                        'user_notes': '',
                        'images_analyzed': len(image_files),
                        'all_image_paths': image_files
                    }
                    
                    verification_data.append(verification_record)
            
            if not verification_data:
                QMessageBox.warning(self, "No Images", "No property images found in adt_detections folder.")
                return
            
            # Sort by address for consistent ordering
            verification_data.sort(key=lambda x: x['address'])
            ...
            QMessageBox.information(self, "Images Loaded", 
                f"Loaded {len(self.verification_data)} property images for verification.\n\n"
                f"These are all the images the AI analyzed but found no ADT signs.\n"
                f"You can now manually verify if the AI was correct.\n\n"
                f"Use the verification buttons to mark:\n"
                f"✅ ADT Present - if you see ADT signs the AI missed\n"
                f"❌ No ADT - if you confirm no ADT signs\n"
                f"❓ Unsure - if you're not certain")
            
            print(f"[DEBUG] Successfully loaded {len(self.verification_data)} non-detected images for verification")

        except Exception as e:
            error_msg = f"Failed to load non-detected images: {e}"
            print(f"[ERROR] {error_msg}")
            QMessageBox.critical(self, "Error", error_msg) 