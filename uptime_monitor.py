...
def ping_server():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(
                f"✅ {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Server alive: {data.get('status')}"
            )
            return True
        else:
            print(
                f"⚠️  {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Server responded with status {response.status_code}"
            )
            return False
    except Exception as e:
        print(
            f"❌ {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Server ping failed: {e}"
        )
        return False
...
def check_health():
    ...
    try:
        response = requests.get(
            f"{SERVER_URL}/api/health", headers={"X-API-Key": API_KEY}, timeout=10
        )
        if response.status_code == 200:
            data = response.json()
            print(
                f"🏥 Health check: {data.get('status')} - API Key: {data.get('api_key_configured')}"
            )
            return True
        else:
